...
class SVMStrategy(IStrategy):
    ...
    def __init__(self, lookback_period: int = 50, kernel: str = 'rbf',
                 C: float = 1.0, gamma: str = 'scale', min_samples: int = 100):
        self._lookback_period = lookback_period
        self._kernel = kernel
        self._C = C
        self._gamma = gamma
        self._min_samples = min_samples
        self._name = "SVM Strategy"
        self._model = SVC(kernel=kernel, C=C, gamma=gamma, probability=True)
    ...
    def set_params(self, **kwargs) -> None:
        ...
        self._model = SVC(kernel=self._kernel, C=self._C, gamma=self._gamma, probability=True)
        self._is_trained = False
    ...
    def _prepare_target(self, df: pd.DataFrame) -> pd.Series:
        """Prepare target labels (1 for buy, -1 for sell, 0 for hold)."""
        future_return = df['Close'].shift(-1) / df['Close'] - 1
        target = pd.Series(0, index=df.index)
        target[future_return > 0.005] = 1  # Buy if future return > 0.5%
        target[future_return < -0.005] = -1  # Sell if future return < -0.5%
        return target
    ...
    def signal(self, df: pd.DataFrame) -> int:
        """Generate SVM-based trading signal."""
